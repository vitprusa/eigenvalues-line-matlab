...
%WRITE_LATEX The \subfloat float holding both panels.
    ...
    fprintf(fid, ['  \\caption{Eigenvalues of the Sturm--Liouville operator ', ...
        ...
        'Comparison of the reference eigenvalues computed with \\texttt{MATSLISE} ', ...
        'and the eigenvalues computed using various discretisations of the ', ...
        'operator, at several numbers of degrees of freedom (DOF) each; ', ...
        '\\texttt{DST} -- discrete sine transform method, \\texttt{FD} -- finite ', ...
        'differences method, \\texttt{CDM} -- Chebyshev differentiation matrix ', ...
        'method, \\texttt{MBCDM} -- mapped barycentric Chebyshev differentiation ', ...
        'matrix method, \\texttt{Numerov} -- Numerov method, \\texttt{Numerov+AC} ', ...
        '-- Numerov method with the asymptotic correction, \\texttt{LGCC} -- ', ...
        'Legendre--Galerkin method. Both panels repeat the reference.}\n'], ...
        interval_text(cfg), cfg.q_text, cfg.pretty, latex_name(cfg.name));
    fprintf(fid, '  \\label{fig:dof_sweep_%s_colour}\n', cfg.name);
    fprintf(fid, '\\end{figure}\n');
end


function s = interval_text(cfg)
%INTERVAL_TEXT The integration interval as LaTeX, without the enclosing $[ ]$.
    if abs(cfg.a) < 1e-12
        s = '0, \pi';
    else
        s = '-\pi/2, \pi/2';
