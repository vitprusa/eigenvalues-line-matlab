% Eigenvalue DOF sweep for the Paine problem \texttt{paine}: the eigenvalue index $k$
...
\begin{figure}[htbp]
  \centering
  \subfloat[\texttt{DST}, \texttt{FD} and \texttt{CDM}.\label{fig:dof_sweep_paine_colour_a}]{%
    \includegraphics[width=0.8\textwidth]{paine_dof_sweep_colour_a}}
  \\
  \subfloat[\texttt{MBCDM}, \texttt{Numerov}, \texttt{Numerov+AC} and \texttt{LGCC}.\label{fig:dof_sweep_paine_colour_b}]{%
    \includegraphics[width=0.8\textwidth]{paine_dof_sweep_colour_b}}
  \caption{Eigenvalues of the Sturm--Liouville operator $-y'' + q(x)y$ on $[0, \pi]$ with $q(x) = \mathrm{e}^{x}$, the Paine problem \texttt{paine}. Comparison of the reference eigenvalues computed with \texttt{MATSLISE} and the eigenvalues computed using various discretisations of the operator, at several numbers of degrees of freedom (DOF) each; \texttt{DST} -- discrete sine transform method, \texttt{FD} -- finite differences method, \texttt{CDM} -- Chebyshev differentiation matrix method, \texttt{MBCDM} -- mapped barycentric Chebyshev differentiation matrix method, \texttt{Numerov} -- Numerov method, \texttt{Numerov+AC} -- Numerov method with the asymptotic correction, \texttt{LGCC} -- Legendre--Galerkin method. Both panels repeat the reference.}
  \label{fig:dof_sweep_paine_colour}
\end{figure}
